        ...
        <div class="flex flex-col md:flex-row items-center gap-2 w-full md:w-auto">
            ...
            <div class="w-full md:w-auto flex items-center justify-center gap-1 bg-white border border-slate-200 rounded-md px-2 py-0.5">
                <input type="date" bind:value={statsStart} on:change={() => statsRangeType = 'custom'}
                    class="text-[10px] outline-none text-slate-500 w-24 text-center">
                    ...
                    <div class="flex flex-col md:flex-row items-start md:items-center justify-between gap-3">
                        ...
                        <div class="flex gap-2 w-full md:w-auto">
                            <button on:click={() => handleGenerateReport('daily')}
                                ...
                                class="flex-1 md:flex-none px-4 py-2 bg-indigo-600 text-white rounded-lg text-sm font-bold hover:bg-indigo-700 disabled:opacity-50 flex items-center justify-center gap-2 transition">
                                {#if generatingReport}
                            ...
                            <button on:click={() => handleGenerateReport('weekly')}
                                disabled={generatingReport || statsData.list.length === 0}
                                class="flex-1 md:flex-none px-4 py-2 bg-purple-600 text-white rounded-lg text-sm font-bold hover:bg-purple-700 disabled:opacity-50 flex items-center justify-center gap-2 transition">
                                {#if generatingReport}
                                    <i class="ph ph-spinner animate-spin"></i>
                                {:else}
                    ...
                    <div class="px-4 md:px-6 py-3 border-b border-slate-50 flex flex-col md:flex-row justify-between items-start md:items-center gap-2">
                        ...
                        <div class="flex flex-wrap gap-1">
                            {#each chartTypes as chart}
                                <button on:click={() => activeChartType = chart.id}
                                    class="px-2 py-1 text-[10px] rounded font-bold transition"
                                    class:bg-indigo-100={activeChartType === chart.id}
                                    class:text-indigo-700={activeChartType === chart.id}
                                    class:bg-slate-50={activeChartType !== chart.id}
                                    class:text-slate-500={activeChartType !== chart.id}>
                                    <i class="ph {chart.icon}"></i>
                                    <span class="hidden md:inline ml-1">{chart.name}</span>
                                </button>
                            {/each}
                        </div>
                    </div>
                    <div class="p-4 h-[250px] md:h-[300px]">
                        {#if statsData.list.length === 0}
                        ...
                        {:else}
                            {#key activeChartType}
                                <Charts tasks={statsData.list} type={activeChartType} />
                            {/key}
                        {/if}
                    </div>
                </div>
...
{#if showReport}
    <div class="fixed inset-0 z-50 flex items-center justify-center p-4">
        <div on:click={closeReport} on:keydown={closeReport} role="button" tabindex="0" class="absolute inset-0 bg-slate-900/40 backdrop-blur-sm"></div>
