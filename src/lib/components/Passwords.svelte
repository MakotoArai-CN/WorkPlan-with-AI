<script>
    import { passwordsStore, isPasswordsUnlocked, passwordsList, passwordCategories, rememberSession } from '../stores/passwords.js';
    ...
    function setupMasterPassword() {
        if (masterPassword.length < 8) {
            showAlert({ title: '密码太短', message: '主密码至少需要8个字符', variant: 'warning' });
            return;
        }
        if (masterPassword !== confirmPassword) {
            showAlert({ title: '密码不匹配', message: '两次输入的密码不一致', variant: 'warning' });
            return;
        }
    ...
    function unlockVault() {
        if (passwordsStore.unlock(unlockPassword)) {
            ...
            showAlert({ title: '密码错误', message: '主密码不正确', variant: 'danger' });
        }
    }
    ...
    function addPassword() {
        if (!newPassword.title || !newPassword.password) {
            showAlert({ title: '信息不完整', message: '标题和密码不能为空', variant: 'warning' });
            return;
        }
    ...
    function generateNewPassword(target = 'new') {
        const generated = generatePassword(passwordGenOptions.length, passwordGenOptions);
        if (target === 'new') {
    ...
    function openExportModal() {
        if (selectMode && selectedPasswordIds.size === 0) {
            showAlert({ title: '未选择密码', message: '请至少选择一条密码记录', variant: 'warning' });
            return;
        }
        closeExportMenu();
    ...
    async function handleExport(format, useEncryption = false) {
        ...
        const idsToExport = selectMode && selectedPasswordIds.size > 0 
        ...
        const masterPass = useEncryption ? passwordsStore.getMasterPassword() : '';
        ...
        try {
            if (format === 'csv') {
                ...
                const result = exportToCSV(data, `passwords_${timestamp}.csv`, { showToast });
    ...
    async function handleImportFile(event) {
        ...
        try {
            if (fileName.endsWith('.xlsx') || fileName.endsWith('.xls')) {
                ...
                showToast({ message: '不支持的文件格式，支持 Excel、CSV、JSON', type: 'error' });
            }
        } catch (e) {
    ...
    async function importFromJSON(file) {
        ...
        try {
            ...
            showToast({ message: 'JSON 解析失败: ' + e.message, type: 'error' });
        }
    }
    ...
    function verifyImportPassword() {
        ...
        try {
            const decrypted = CryptoJS.AES.decrypt(importData.data, importPasswordInput).toString(CryptoJS.enc.Utf8);
            if (!decrypted) {
            ...
            importAttempts++;
            if (importAttempts >= 3) {
                showToast({ message: '密码错误次数过多，请重新选择文件', type: 'error' });
                showImportPasswordModal = false;
                importData = null;
                importAttempts = 0;
            } else {
                showToast({ message: `密码错误，还剩 ${3 - importAttempts} 次机会`, type: 'error' });
            }
            importPasswordInput = '';
        }
    }
    ...
    async function processImportedDataAsync(data) {
        ...
        const validEntries = data.filter(item => {
            ...
            title: item.title || item['标题'] || item.Title || item.name || item.Name || '',
            username: item.username || item['用户名'] || item.Username || item.login || item.Login || '',
...
<input type="file" bind:this={importFileInput} on:change={handleImportFile}
    accept=".xlsx,.xls,.csv,.json" class="hidden">
...
<div class="flex flex-col h-screen md:h-full overflow-hidden bg-slate-50">
    <header class="h-14 md:h-16 bg-white/90 backdrop-blur px-4 md:px-6 flex justify-between items-center z-10 sticky top-0 border-b border-slate-200 shrink-0">
        <div class="flex items-center gap-2">
            {#if isMobile && $isPasswordsUnlocked}
                <button on:click={() => showSidebar = !showSidebar} class="p-2 text-slate-500 hover:bg-slate-100 rounded-lg">
                    <i class="ph ph-funnel text-xl"></i>
                </button>
            {/if}
            <div>
                <h2 class="text-base md:text-lg font-bold text-amber-800">密码记录本</h2>
        ...
        {#if $isPasswordsUnlocked}
            <div class="flex items-center gap-1 md:gap-2">
                ...
                {/if}
                <div class="relative">
                    <button on:click={toggleExportMenu}
                        class="export-menu-button h-9 px-3 bg-slate-100 hover:bg-slate-200 rounded-lg text-slate-600">
                        <i class="ph ph-dots-three-vertical"></i>
                    </button>
                    {#if showExportMenu}
                        <div class="export-menu-dropdown absolute right-0 top-full mt-1 bg-white border border-slate-200 rounded-lg shadow-lg py-1 z-50 min-w-[150px]">
                            <button on:click={toggleSelectMode}
                                class="w-full text-left px-4 py-2.5 text-sm hover:bg-slate-50 flex items-center gap-2">
                                <i class="ph ph-check-square text-lg text-blue-600"></i> 
                                {selectMode ? '退出选择' : '选择导出'}
                            </button>
                            <button on:click={openExportModal}
                                class="w-full text-left px-4 py-2.5 text-sm hover:bg-slate-50 flex items-center gap-2">
                                <i class="ph ph-export text-lg text-green-600"></i> 导出密码
                            </button>
                            <button on:click={triggerImport}
                                class="w-full text-left px-4 py-2.5 text-sm hover:bg-slate-50 flex items-center gap-2">
                                <i class="ph ph-download text-lg text-purple-600"></i> 导入密码
                            </button>
                            <hr class="my-1 border-slate-100">
                            <button on:click={toggleRememberSession}
                                class="w-full text-left px-4 py-2.5 text-sm hover:bg-slate-50 flex items-center gap-2">
                                <i class="ph {$rememberSession ? 'ph-check-square' : 'ph-square'} text-lg text-indigo-600"></i> 
                                记住会话
                            </button>
                            <button on:click={lockVault}
                                class="w-full text-left px-4 py-2.5 text-sm hover:bg-slate-50 flex items-center gap-2">
                                <i class="ph ph-lock text-lg text-slate-500"></i> 锁定
                            </button>
                            <button on:click={clearAllPasswords}
    ...
    {#if showImportProgress}
        <div class="bg-purple-50 border-b border-purple-200 px-4 py-3">
            <div class="flex items-center justify-between mb-2">
                <span class="text-sm text-purple-700 font-bold">正在导入密码...</span>
                <span class="text-xs text-purple-600">{importProgress} / {importTotal}</span>
            </div>
            <div class="w-full bg-purple-200 rounded-full h-2">
                <div class="bg-purple-600 h-2 rounded-full transition-all duration-100"
                    style="width: {(importProgress / importTotal) * 100}%"></div>
            </div>
        </div>
    {/if}
    ...
    {#if $isPasswordsUnlocked}
        <div class="flex flex-1 overflow-hidden relative">
            ...
            <div class="w-48 md:w-56 bg-white border-r border-slate-200 flex flex-col shrink-0 absolute md:relative z-30 h-full transition-transform duration-200"
                class:translate-x-0={!isMobile || showSidebar}
                class:-translate-x-full={isMobile && !showSidebar}>
                <div class="p-3 border-b border-slate-100">
                    <input type="text" bind:value={searchQuery} placeholder="搜索..."
                ...
                <div class="flex-1 overflow-y-auto py-2">
                    <button on:click={() => selectCategory('全部')}
                    ...
                    {#each $passwordCategories as category}
                        <button on:click={() => selectCategory(category)}
                            ...
                            class:hover:bg-slate-50={selectedCategory !== category}>
                            <i class="ph ph-folder mr-2"></i> {category}
                        </button>
                    {/each}
                </div>
            </div>

            <div class="flex-1 overflow-y-auto p-4 overscroll-contain" style="-webkit-overflow-scrolling: touch;">
                {#if filteredPasswords.length === 0}
                    <div class="flex items-center justify-center h-64 text-slate-400">
                        <div class="text-center">
                            ...
                            {#if isMobile}
                                <button on:click={() => showSidebar = true} class="mt-4 text-amber-600 font-bold text-sm">
                ...
                {:else}
                    <div class="password-grid pb-4">
                        {#each displayedPasswords as password, index (password.id)}
                            <div class="bg-white rounded-xl border border-slate-200 p-4 shadow-sm hover:shadow-md transition flex flex-col password-card"
                                class:ring-2={selectMode && selectedPasswordIds.has(password.id)}
                                class:ring-amber-400={selectMode && selectedPasswordIds.has(password.id)}
                                style="animation-delay: {Math.min(index % 50, 10) * 30}ms">
                                <div class="flex items-start justify-between mb-3">
                                    <div class="flex-1 min-w-0 flex items-center gap-2">
                                        {#if selectMode}
                                            <button on:click={() => togglePasswordSelection(password.id)}
                                                class="shrink-0">
                                                <i class="ph {selectedPasswordIds.has(password.id) ? 'ph-check-square' : 'ph-square'} text-2xl text-amber-600"></i>
                                            </button>
                                        {/if}
                                        <div class="flex-1 min-w-0">
                                            <h3 class="font-bold text-slate-800 text-base mb-1 truncate">{password.title}</h3>
                                            <span class="text-xs px-2 py-1 bg-amber-100 text-amber-700 rounded">{password.category}</span>
                                        </div>
                                    </div>
                                    {#if !selectMode}
                                        <div class="flex gap-1 shrink-0 ml-2">
                                            <button on:click={() => openEditModal(password)}
                                                class="p-2 text-slate-400 hover:text-blue-600 hover:bg-blue-50 rounded-lg">
                                                <i class="ph ph-pencil-simple"></i>
                                            </button>
                                            <button on:click={() => deletePassword(password.id)}
                                ...
                                <div class="space-y-2 text-sm flex-1">
                                    ...
                                    <div class="flex items-center gap-2 h-7">
                                        ...
                                        <button on:click={() => togglePasswordVisibility(password.id, password.password)}
                                            class="text-slate-400 hover:text-blue-600 shrink-0 p-1">
                                            <i class="ph ph-eye{showPasswordValue[password.id] ? '-slash' : ''}"></i>
                                        </button>
                                        <button on:click={() => copyPassword(password.id, password.password)}
                                            class="text-slate-400 hover:text-blue-600 shrink-0 p-1">
                                            <i class="ph ph-copy"></i>
                                        </button>
                                    </div>
                                    ...
                                    {#if password.notes}
                                        <div class="flex items-start gap-2">
                                            <i class="ph ph-note text-slate-400 mt-0.5 shrink-0"></i>
                                            <span class="text-slate-500 text-xs line-clamp-2">{password.notes}</span>
                                        </div>
                                    {/if}
                                </div>
                            </div>
                    ...
                    {#if hasMore}
                        <div bind:this={loadTrigger} class="flex justify-center py-8 pb-20">
                            {#if loadingMore}
                            ...
                            {:else}
                                <div class="text-sm text-slate-400">向下滚动加载更多</div>
                            {/if}
                        </div>
                    {:else}
                        <div class="pb-20"></div>
...
{#if showImportPasswordModal}
    <div class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-sm">
        <div class="bg-white rounded-2xl shadow-2xl w-full max-w-md p-6">
            ...
            <p class="text-sm text-slate-600 mb-4">检测到加密的 JSON 文件，请输入密码解密：</p>
            <input type="password" bind:value={importPasswordInput}
            ...
            <p class="text-xs text-slate-500 mb-4">剩余尝试次数：{3 - importAttempts}</p>
            <div class="flex gap-3">
                <button on:click={cancelImportPassword}
                    class="flex-1 py-2.5 bg-slate-100 text-slate-600 rounded-lg font-bold hover:bg-slate-200">
                    取消
                </button>
                <button on:click={verifyImportPassword}
    ...
    <div class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-sm">
        <div class="bg-white rounded-2xl shadow-2xl w-full max-w-md p-6">
            ...
            <div class="space-y-3 mb-6">
                <button on:click={() => handleExport('csv', false)}
                    ...
                    <div class="flex-1 text-left">
                        ...
                        <div class="text-xs text-slate-500">Excel兼容，明文存储</div>
                    </div>
                </button>
                <button on:click={() => handleExport('json', true)}
            ...
            <button on:click={() => showExportModal = false}
...
{#if showSetupModal}
    <div class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-sm">
        <div class="bg-white rounded-2xl shadow-2xl w-full max-w-md p-6">
            ...
            <p class="text-sm text-slate-600 mb-4">主密码用于加密所有密码记录，请妥善保管，丢失后无法恢复！</p>
            <div class="space-y-3">
                <div>
                    <label class="text-xs font-bold text-slate-500 uppercase mb-1 block">主密码（至少8位）</label>
                    <input type="password" bind:value={masterPassword} placeholder="输入主密码"
                        class="w-full border border-slate-200 rounded-lg px-3 py-2.5 focus:outline-none focus:border-amber-400">
                </div>
                <div>
                    <label class="text-xs font-bold text-slate-500 uppercase mb-1 block">确认密码</label>
                    <input type="password" bind:value={confirmPassword} placeholder="再次输入主密码"
                        on:keydown={(e) => e.key === 'Enter' && setupMasterPassword()}
                        class="w-full border border-slate-200 rounded-lg px-3 py-2.5 focus:outline-none focus:border-amber-400">
                </div>
            </div>
            <button on:click={setupMasterPassword}
...
{#if showUnlockModal}
    <div class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-sm">
        <div class="bg-white rounded-2xl shadow-2xl w-full max-w-md p-6">
            ...
            <div class="space-y-3">
                <div>
                    <label class="text-xs font-bold text-slate-500 uppercase mb-1 block">主密码</label>
            ...
            <button on:click={unlockVault}
                class="w-full mt-4 py-3 bg-amber-600 text-white rounded-lg font-bold hover:bg-amber-700">
                解锁
            </button>
            <button on:click={handleForgotPassword}
...
{#if showAddModal}
    <div class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-sm">
        <div class="bg-white rounded-2xl shadow-2xl w-full max-w-lg p-6 max-h-[90vh] overflow-y-auto">
            ...
            <div class="space-y-3">
                <div>
                    <label class="text-xs font-bold text-slate-500 uppercase mb-1 block">标题 *</label>
                    <input type="text" bind:value={newPassword.title} placeholder="例如：Google 账号"
                        class="w-full border border-slate-200 rounded-lg px-3 py-2 focus:outline-none focus:border-amber-400">
                </div>
                <div>
                    <label class="text-xs font-bold text-slate-500 uppercase mb-1 block">用户名/邮箱</label>
                    <input type="text" bind:value={newPassword.username} placeholder="username@example.com"
                        class="w-full border border-slate-200 rounded-lg px-3 py-2 focus:outline-none focus:border-amber-400">
                </div>
                <div>
                    <label class="text-xs font-bold text-slate-500 uppercase mb-1 block flex justify-between items-center">
                        <span>密码 *</span>
                        <button on:click={() => generateNewPassword('new')} class="text-blue-600 text-xs font-bold">
                            <i class="ph ph-magic-wand"></i> 生成
                        </button>
                    </label>
                    <input type="text" bind:value={newPassword.password} placeholder="密码"
                        class="w-full border border-slate-200 rounded-lg px-3 py-2 focus:outline-none focus:border-amber-400 font-mono">
                </div>
                <div>
                    <label class="text-xs font-bold text-slate-500 uppercase mb-1 block">网址</label>
                    <input type="url" bind:value={newPassword.url} placeholder="https://example.com"
                        class="w-full border border-slate-200 rounded-lg px-3 py-2 focus:outline-none focus:border-amber-400">
                </div>
                <div>
                    <label class="text-xs font-bold text-slate-500 uppercase mb-1 block">分类</label>
                    <select bind:value={newPassword.category}
                        class="w-full border border-slate-200 rounded-lg px-3 py-2 focus:outline-none focus:border-amber-400">
                ...
                <div>
                    <label class="text-xs font-bold text-slate-500 uppercase mb-1 block">备注</label>
                    <textarea bind:value={newPassword.notes} rows="3" placeholder="其他信息..."
                        class="w-full border border-slate-200 rounded-lg px-3 py-2 focus:outline-none focus:border-amber-400 resize-none"></textarea>
                </div>
            </div>
    ...
    <div class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-sm">
        <div class="bg-white rounded-2xl shadow-2xl w-full max-w-lg p-6 max-h-[90vh] overflow-y-auto">
            ...
            <div class="space-y-3">
                <div>
                    <label class="text-xs font-bold text-slate-500 uppercase mb-1 block">标题 *</label>
                    <input type="text" bind:value={editingPassword.title}
                        class="w-full border border-slate-200 rounded-lg px-3 py-2 focus:outline-none focus:border-amber-400">
                </div>
                <div>
                    <label class="text-xs font-bold text-slate-500 uppercase mb-1 block">用户名/邮箱</label>
                    <input type="text" bind:value={editingPassword.username}
                        class="w-full border border-slate-200 rounded-lg px-3 py-2 focus:outline-none focus:border-amber-400">
                </div>
                <div>
                    <label class="text-xs font-bold text-slate-500 uppercase mb-1 block flex justify-between items-center">
                        <span>密码 *</span>
                        <button on:click={() => generateNewPassword('edit')} class="text-blue-600 text-xs font-bold">
                            <i class="ph ph-magic-wand"></i> 生成
                        </button>
                    </label>
                    <input type="text" bind:value={editingPassword.password}
                        class="w-full border border-slate-200 rounded-lg px-3 py-2 focus:outline-none focus:border-amber-400 font-mono">
                </div>
                <div>
                    <label class="text-xs font-bold text-slate-500 uppercase mb-1 block">网址</label>
                    <input type="url" bind:value={editingPassword.url}
                        class="w-full border border-slate-200 rounded-lg px-3 py-2 focus:outline-none focus:border-amber-400">
                </div>
                <div>
                    <label class="text-xs font-bold text-slate-500 uppercase mb-1 block">分类</label>
                    <select bind:value={editingPassword.category}
                        class="w-full border border-slate-200 rounded-lg px-3 py-2 focus:outline-none focus:border-amber-400">
                        {#each $passwordCategories as cat}
                            <option value={cat}>{cat}</option>
                        {/each}
                ...
                <div>
                    <label class="text-xs font-bold text-slate-500 uppercase mb-1 block">备注</label>
                    <textarea bind:value={editingPassword.notes} rows="3"
                        class="w-full border border-slate-200 rounded-lg px-3 py-2 focus:outline-none focus:border-amber-400 resize-none"></textarea>
                </div>
            </div>
            <div class="flex gap-3 mt-4">
                <button on:click={() => { showEditModal = false; editingPassword = null; }}
                    class="flex-1 py-2.5 bg-slate-100 text-slate-600 rounded-lg font-bold hover:bg-slate-200">
                    取消
                </button>
